...
#text = ' '.join([str(i) for i in range(1500,2025)])


def plot_embeddings_with_pca(embeddings, text, which_lvls=[1,1,1,1], filename="vis_embs.png", annotate=True):
    ...
    # Flatten the embeddings for PCA or t-SNE
    ...
    if do_subwords:
        embeddings_to_reduce.append(subword_embs)
    if do_words:
    ...
    #reduced_embs = PCA(n_components=2).fit_transform(all_embs)
    ...
    current_index = 0
    if do_subwords:
        subword_len = len(subword_embs)
        subword_indices = np.arange(current_index, current_index + subword_len)
    ...
    # Subword embeddings: tiny gray dots
    if do_subwords:
        plt.scatter(reduced_embs[subword_indices, 0], reduced_embs[subword_indices, 1], 
                    s=10, c='gray', label='Subword Embeddings', alpha=0.5)
    ...
    # Phrase3 embeddings: biggest blue dots
    if do_phrase3s:
...
from infer import text_emb_lvl

model = PyRvNN()
model.load_weights("./checkpoints/a1")

print(text)
embs = text_emb_lvl(model, text, 2, multilvl=True)
plot_embeddings_with_pca(embs, text, which_lvls=[0,1,0,0], filename="vis_embs.png", annotate=True)
